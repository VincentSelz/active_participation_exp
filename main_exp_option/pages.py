...
class Option(Page):
    form_model = 'player'
    form_fields = ['option_accepted']

class OptionWait(Page):
    timeout_seconds = Constants.task_timeout
    def is_displayed(self):
        player = self.player
        return player.option_accepted == 1
    # Apply the outside option and pass the main task (via timeout=0)
    def before_next_page(self):
        me = self.player
        me.current_max_is = Constants.outside_offer
        participant = self.participant
        import time
        participant.vars['expiry'] = time.time() + 0


class TrueStart(Page):
    def is_displayed(self):
        player = self.player
        return player.option_accepted == 0
    def before_next_page(self):
        participant = self.participant
        import time
